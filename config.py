...
MIN_MARKET_DATA_SUCCESS_RATIO = 0.6
...
STAGE1_PROMPT = """당신은 금융 데이터 분석가입니다. 아래 시장 데이터와 뉴스 헤드라인만을 근거로 사실을 추출하세요.
...
2. 거시경제 관련 사실 (헤드라인 근거, 키워드 표기)
3. 섹터별 사실 (반도체/소부장, 2차전지, 자동차, 부동산, 바이오)
4. 워치리스트 종목 관련 사실 (종목명별, 헤드라인에 언급된 것만)
5. 확인되지 않은 정보 (데이터 부족 시 명시)
"""

STAGE2_PROMPT = """당신은 전문 주식 애널리스트입니다. 아래 1단계 사실 추출 결과만을 근거로 상세 분석 리포트를 작성하세요.
...
파트 3:
- 🎯 워치리스트 종목별 브리핑 (9종목 각 2~3줄: 삼성전자, SK하이닉스, 현대차, 네이버, 한미반도체, 주성엔지니어링, 리노공업, HPSP, 솔브레인)
- 💡 투자 인사이트 (긍정/부정 요인, 사실과 해석 구분)
- ⚠️ 주의: 본 리포트는 정보 제공 목적이며 투자 권유가 아닙니다.
"""
